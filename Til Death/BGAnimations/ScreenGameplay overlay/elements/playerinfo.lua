...
local translated_info = TranslationMatrices["SGPlayerInfo"]
	...
	LoadFont("Common Large") .. {
		...
		SetCommand = function(self)
		   local steps = GAMESTATE:GetCurrentSteps()
			self:settext(ShortDiffToFancyLUT[GetDifficultyNameFromSteps(steps)])
			self:diffuse(
				getDifficultyColor(
					GetCustomDifficulty(
						steps:GetStepsType(),
						GetDifficultyNameFromSteps(steps, true)
					)
				)
			)
		end,
