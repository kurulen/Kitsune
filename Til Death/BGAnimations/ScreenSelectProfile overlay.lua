local translated_info = TranslationMatrices["ScreenSelectProfile"]
	...
	for p = 0, PROFILEMAN:GetNumLocalProfiles() - 1 do
		...
		local ProfileCard = Def.ActorFrame {
			...
			LoadFont("Common Large") ..  {
				...
				InitCommand = function(self)
					self:xy(38 / 2, -12)
					self:settextf("%s: %.2f", profile:GetDisplayName(), profile:GetPlayerRating())
					self:zoom(0.4):ztest(true):maxwidth((260 - 40 - 4) / 0.4)
			...
			LoadFont("Common Normal") ..  {
				...
				BeginCommand = function(self)
					...
					local plural = true
					if numSongsPlayed then
						if numSongsPlayed < 10 then
							plural = false
						end
					end
					self:settextf("%i %s", numSongsPlayed, (plural and translated_info["SongsPlayed"] or translated_info["SongPlayed"]))
				end
			},
			Def.Sprite {
				InitCommand = function(self)
