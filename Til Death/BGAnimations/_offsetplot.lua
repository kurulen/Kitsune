...
local plotX, plotY = SCREEN_WIDTH - 6 - plotWidth / 2, SCREEN_HEIGHT - 59.5 - plotHeight / 2
...
local translated_info = TranslationMatrices["OffsetPlot"]
...
o[#o + 1] = Def.Quad {
	...
	JudgeDisplayChangedMessageCommand = function(self)
		...
		if string.find(SCREENMAN:GetTopScreen():GetName(), "Screen.*Evaluation") then
		   self:zoomto(plotWidth + plotMargin, plotHeight + plotMargin + 40)
		end
...
o[#o + 1] = Def.Quad {
	JudgeDisplayChangedMessageCommand = function(self)
		self:zoomto(plotWidth + plotMargin, 1):diffuse(getJudgmentColor("TapNoteScore_W1")):diffusealpha(baralpha)
	end
}
...
for i = 1, #fantabars do
	o[#o + 1] = Def.Quad {
		JudgeDisplayChangedMessageCommand = function(self)
			self:zoomto(plotWidth + plotMargin, 1):diffuse(getJudgmentColor(bantafars[i])):diffusealpha(baralpha)
