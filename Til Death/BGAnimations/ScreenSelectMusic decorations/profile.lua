...
local translated_info = TranslationMatrices["Profile"]
...
local t = Def.ActorFrame {
	...
	SetCommand = function(self)
		...
		if getTabIndex() == 4 or SCREENMAN:GetTopScreen():GetName() == "ScreenNetRoom" and getTabIndex() == 1 then
		        CurrentSubTidbit = "ProfileDisplay"
...
local function byValidity(valid)
	...
	return getJudgmentColor("TapNoteScore_Miss")
end
	...
	local t = Def.ActorFrame {
		...
		LoadFont("Common Large") .. {
			...
			DisplayProfileRankingLabelsMessageCommand = function(self)
				...
				if not showOnline then
					if thssteps then
						local diff = GetDifficultyNameFromSteps(thssteps, true)
						self:diffuse(getDifficultyColor(diff))
						self:settext(DiffToShortDiffLUT[diff])
					else
						self:settext("")
					end
				else
