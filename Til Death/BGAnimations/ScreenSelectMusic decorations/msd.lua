...
t[#t + 1] = LoadFont("Common Normal") .. {
	...
	SetCommand = function(self)
		steps = GAMESTATE:GetCurrentSteps()
		if steps ~= nil then
			local diff = GetDifficultyNameFromSteps(steps, true)
			local stype = steps:GetStepsType()
			local d_stype = ToEnumShortString(stype):gsub("%_", " ")
			local meter = steps:GetMeter()
			if update then
				self:settext(d_stype .. " " .. DiffToShortDiffLUT[diff] .. " " .. meter)
...
t[#t + 1] = LoadFont("Common Normal") .. {
	...
	SetCommand = function(self)
		...
		if steps ~= nil and song ~= nil and update then
			...
			self:diffuse(Saturation(getDifficultyColor(GetCustomDifficulty(steps:GetStepsType(), GetDifficultyNameFromSteps(steps, true))), 0.3))
		else
			self:settext("")
		end
	end
