...
local function valtexact(name, x, y, c)
	return Def.ActorFrame {
		...
		Def.BitmapText {
			...
			Name = "Name",
			InitCommand = function(self)
				self:x(-4):settext(OverscanPreferenceToHumanNameEnum[name] .. "(" .. valtex_buttons[name][2] .. " or " .. valtex_buttons[name][1] .. "): "):horizalign(right):diffuse(c)
			end
		}
	}
end
...
local function input(event)
	...
	for valname, button_set in pairs(valtex_buttons) do
		if button == string.lower(button_set[1]) then
			change_center_val(valname, 1)
		elseif button == string.lower(button_set[2]) then
			change_center_val(valname, -1)
		end
	end
end
...
local t = Def.ActorFrame {
	...
	LoadFont("Common Large") .. {
	   InitCommand = function(self)
	      self:Center()
	      self:zoom(0.35)
	      self:settext("This settings page is meant for CRT displays!\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\nIf you don't know what this is,\njust press <Esc>.")
	   end
	}
}
