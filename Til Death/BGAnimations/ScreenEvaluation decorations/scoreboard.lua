...
local function input(event)
	...
	if event.type == "InputEventType_FirstPress" and scoreBoard then
		...
		if event.DeviceInput.button == "DeviceButton_u" and holdingCtrl then
		   ...
		   local si = scoreBoard:GetChild("scoreItem"..indexedButton)
		   si:playcommand("ParadiseCity")
		elseif event.DeviceInput.button == "DeviceButton_i" and holdingCtrl then
		   indexedButton = indexedButton + 1
		   if indexedButton < 0 then
		      indexedButton = 0
		   elseif indexedButton > (lines - 1) then
		      indexedButton = lines - 1
		   end
		   local si = scoreBoard:GetChild("scoreItem"..indexedButton)
		   si:playcommand("ParadiseCity")
		elseif event.DeviceInput.button == "DeviceButton_space" and holdingCtrl then
		   local si = scoreBoard:GetChild("scoreItem"..indexedButton)
		   si:playcommand("ShowMeWhatYouGot")
		elseif event.DeviceInput.button == "DeviceButton_n" and holdingCtrl then
		   MESSAGEMAN:Broadcast("WheelDownSlow")
		elseif event.DeviceInput.button == "DeviceButton_m" and holdingCtrl then
...
local function scoreitem(pn, index, scoreIndex, drawindex)
	...
	local t = Def.ActorFrame {
		...
		Def.Quad {
			...
			ParadiseCityCommand = function(self, params)
			...
			ShowMeWhatYouGotCommand = function(self)
				local p = self:GetParent()
				local grade = p:GetChild("grade")
				local judge = p:GetChild("judge")
				local date = p:GetChild("date")
